...
public class DeviceInfoSync {
    ...
    public static func executeQueryInPHP(query: String, tablename: String, counti: Int, completion: @escaping (String?, Int?, [String: Any]?, Bool, String?) -> Void) {
        let url = URL(string: "http://localhost/RaghulMonishsNeuralEngine/iOS/DeviceFingerprints/DeviceInfoSync.php")! // Replace with your server URL
    ...
    public static func segregateAndPrintQueries(_ output: (Bool, Any?)) {
        ...
        for queryDict in queries {
            if let query = queryDict["query"],
               ...
               let countioftableStr = queryDict["countioftable"],
               let countioftable = Int(countioftableStr),
               let countiStr = queryDict["counti"],
               let counti = Int(countiStr) {
                queryArray.append((query, tablename, countioftable, counti))
            }
        }
    ...
    private static func executeQueries(queries: [(String, String, Int, Int)]) {
        ...
        for (query, tablename, countioftable, counti) in queries {
            ...
            currentBatch.append((query, tablename, countioftable, counti))
        }
        ...
        dispatchGroup.notify(queue: .main) {
            if !combinedFailedMessages.isEmpty {
                ...
                clSync("DeviceInfoSync Response: \(finalResponse)")
            }
        }
    }
    ...
    private static func executeBatch(_ batch: [(String, String, Int, Int)], completion: @escaping ([String], [String]) -> Void) {
        ...
        for (query, tablename, countioftable, counti) in batch {
            dispatchGroup.enter()
            executeQueryInPHP(query: query, tablename: tablename, counti: countioftable) { response, lastId, json, success, errorMessage in
                if success {
