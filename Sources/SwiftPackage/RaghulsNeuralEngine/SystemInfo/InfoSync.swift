...
public class SystemInfoSync {
    ...
    public static func executeQueryInPHP(query: String, tablename: String, counti: Int, completion: @escaping (String?, Int?, [String: Any]?, Bool, String?) -> Void) {
        let url = URL(string: "http://localhost/RaghulMonishsNeuralEngine/iOS/IPDataInfoSync.php")! // Replace with your server URL
    ...
    public static func segregateAndPrintQueries(_ output: (Bool, Any?)) {
        guard let (_, queries) = output as? (Bool, [[String: String]]), !queries.isEmpty else {
            print("Invalid output or no queries found.")
            return
        }
    ...
    private static func executeQueries(queries: [(String, String, Int, Int)]) {
        ...
        for (query, tablename, countioftable, counti) in queries {
            ...
            if currentBatchSize + querySize > 200 * 1024 {
                print("Current batch size: \(currentBatchSize / 1024) KB")
    ...
    private static func executeBatch(_ batch: [(String, String, Int, Int)], completion: @escaping ([String], [String]) -> Void) {
        ...
        for (query, tablename, countioftable, counti) in batch {
            dispatchGroup.enter()
            executeQueryInPHP(query: query, tablename: tablename, counti: counti) { response, lastId, json, success, errorMessage in
                if success {
